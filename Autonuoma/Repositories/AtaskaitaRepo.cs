...
public class AtaskaitaRepo : RepoBase
{
	public static List<ServicesReport.Paslauga> GetServicesOrdered(DateTime? dateFrom, DateTime? dateTo)
	{
		var query =
			...
			FROM `{Config.TblPrefix}paslaugos` pasl
			INNER JOIN `{Config.TblPrefix}uzsakytos_paslaugos` up ON pasl.id = up.fk_paslauga
			INNER JOIN `{Config.TblPrefix}sutartys` sut ON up.fk_sutartis = sut.nr
			WHERE
			...
			FROM
				...
				LEFT JOIN
					(
						...
						FROM `{Config.TblPrefix}sutartys` sut1
						INNER JOIN `{Config.TblPrefix}klientai` kln1 ON kln1.asmens_kodas=sut1.fk_klientas
						WHERE
							sut1.sutarties_data >= IFNULL(?nuo, sut1.sutarties_data)
	...
	public static List<LateContractsReport.Sutartis> GetLateReturnContracts(DateTime? dateFrom, DateTime? dateTo)
	{
		var query =
			...
			FROM `{Config.TblPrefix}sutartys` sut
			INNER JOIN `{Config.TblPrefix}klientai` kln ON kln.asmens_kodas = sut.fk_klientas
			WHERE
				(
